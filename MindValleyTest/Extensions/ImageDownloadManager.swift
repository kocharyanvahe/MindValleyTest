...
class ImageDownloadManager: NSObject {
    ...
    func getImageFromURL(imageURLString:String, completionHandler:@escaping DownloadHandler) {
    ...
    func getImageFromURL(imageURLString: String, progessHandler: @escaping DownloadProgressHandler, completionHandler: @escaping DownloadHandler) {
        if let cachedImage = imageCache.object(forKey: imageURLString as NSString) as UIImage? {
            completionHandler(cachedImage, nil)
            return
        }
    ...
    private func downloadImageFor(imageURLString: String, downloadHandler: @escaping DownloadHandler) {
        ...
        if imageDownloadInfo == nil {
            ...
            imageDownloadInfo = ImageDownloadInfo(downloadURLString: imageURLString, downloadTask: imageLoaderTask, progressHandler: nil, completionHandler: downloadHandler)
            imageLoaderQueue[imageURLString] = imageDownloadInfo
            imageDownloadInfo?.downloadTask.resume()
        }
    }
    
    private func downloadImageFor(imageURLString: String, progressHandler: @escaping DownloadProgressHandler, completionHandler: @escaping DownloadHandler) {
        var imageDownloadInfo: ImageDownloadInfo? = imageLoaderQueue[imageURLString]
        if imageDownloadInfo == nil {
